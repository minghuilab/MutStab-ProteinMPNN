...
def main(args):
    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            if args.conditional_probs_only: # 下面这些为什么要用elif？改为if
                if print_all:
            ...
            if args.embedding:
                if print_all:
                    print(f'Calculating embedding probabilities for {name_}')
                # getting ProteinMPNN structure embeddings
                (Path(base_folder)/'embedding').mkdir(parents=True, exist_ok=True)
                embedding_file = base_folder + '/embedding/' + batch_clones[0]['name']
                embedding_list = []
                hid_list = []
                logits_list = []
                for j in range(NUM_BATCHES):
                    #randn_1 = torch.randn(chain_M.shape, device=X.device)
                    all_mpnn_hid, embedding, logits = model.embedding(X, S, mask, chain_M, residue_idx, chain_encoding_all, None)
                    embedding_list.append(embedding.cpu().numpy())
                    #print([i.shape for i in  all_mpnn_hid])
                    hid_list.append(torch.cat(all_mpnn_hid))
                    logits_list.append(logits)
                concat_log_p = np.concatenate(embedding_list, 0)
                hid_list = np.concatenate(hid_list, 0)
                logits_list = np.concatenate(logits_list, 0)
                mask_out = (chain_M*chain_M_pos*mask)[0,].cpu().numpy()
                m = mask[0,].cpu().numpy().astype(bool)
                #embedding,hid,logits = np.float16(concat_log_p[:,m,:]),np.float16(hid_list[:,m,:]),np.float16(logits_list[:,m,:])
                embedding,hid,logits = concat_log_p[:,m,:],hid_list[:,m,:],logits_list[:,m,:]
                #np.savez(embedding_file, log_p=concat_log_p, S=S[0,].cpu().numpy(), mask=mask[0,].cpu().numpy(), design_mask=mask_out)
                np.savez(embedding_file, embedding = embedding,hid=hid,logits = logits)
            else:
                randn_1 = torch.randn(chain_M.shape, device=X.device)
                log_probs = model(X, S, mask, chain_M*chain_M_pos, residue_idx, chain_encoding_all, randn_1)
                mask_for_loss = mask*chain_M*chain_M_pos
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--embedding", type=int, default=0, help="0 for False, 1 for True; output embedding for residuse;after dencoder")   
